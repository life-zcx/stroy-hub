...
const TELEGRAM_CHAT_ID: string = (process.env.TELEGRAM_ADMIN_CHAT_ID || process.env.TELEGRAM_CHAT_ID)?.trim() || '';
const TELEGRAM_API_BASE: string = process.env.TELEGRAM_API_BASE?.trim() || 'https://api.telegram.org';

// Функция для отправки критических ошибок в Telegram
const sendTelegramError = (message: string, meta: Record<string, any>): void => {
...
// Добавляем консольный вывод, если не в production
